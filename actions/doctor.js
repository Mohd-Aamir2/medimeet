...
export async function setAvailabilitySlots(formData) {
  ...
  try {
    // ✅ use findFirst instead of findUnique (safe with multiple filters)
    ...
    if (existingSlots.length > 0) {
      ...
      if (slotsWithNoAppointments.length > 0) {
        await db.availability.deleteMany({
          where: { id: { in: slotsWithNoAppointments.map((slot) => slot.id) } },
        });
      }
    }
...
export async function getDoctorAppointments() {
  ...
  try {
    ...
    const appointments = await db.appointment.findMany({
      where: {
        doctorId: doctor.id,
        status: { in: ["SCHEDULED", "COMPLETED"] }, // ✅ include completed if desired
      },
...
export async function markAppointmentCompleted(formData) {
  ...
  try {
    ...
    if (now < appointmentEndTime)
      throw new Error("Cannot mark appointment as completed before end time");
